...
Each case contains an operation and an operand (whose magnitute can go up to 5)<br>
...
#### Initial creation
For each grid size, a large amount of levels is generated.<br>

##### Best Score And Best Soltuion Search
The strategy here is to try every possible solution, and keep the set of moves that leads to the highest score.<br>
For the two first grid size, the script is quite fast<br>
...
##### Duplicate checks
There is a probability (low, but not 0) that te same level  have been generated several times.<br>
For that reason, the initial set of levels goes trough a function that checks for duplicates, and remove the duplicated ones.<br>

##### Extreme Removal
Some levels will have what we will refer to a "Extreme" solution, i.e. a solution which is either very long, very short, leads to a score that is very high, or very low. <br>
An exagerated example would be a level whose best solution is 1. That would mean that any solution would reduce the score below 1.<br>
...
##### Difficulty
The difficulty of each levels is then estimated. It is not an easy task, as it is quite subtle (what is a difficult level ?).<br>
The levels have a parameter called estimated_difficulty, which is a float between 0 and 1<br>
For now, this is a combinaison of two factors : <br>
...
- How much the score will decrease trough the best solution (in that case, we only look at the "negatives" operations) (again, the idea is that a best solution that goes trough a lot of diminution will be hard to see)<br>
