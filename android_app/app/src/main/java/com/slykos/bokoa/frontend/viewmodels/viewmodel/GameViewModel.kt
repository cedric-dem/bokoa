...
class GameViewModel(private val userRepository: UserRepository) : ViewModel() {
    ...
    private lateinit var page: GamePageHandler
    ...
    fun startGame(page: GamePageHandler, level: Int, diff: Int) {
        this.page = page
        levelId = level
        difficulty = diff

        currentGame = RealGame(page) { scoreString ->
            _topText.value = page.getString(R.string.score)
    ...
    private fun initLevel() {
        ...
        currentLevel = loadLevelFromJson(page, fileName)!!
    ...
    private fun runGame() {
        initLevel()
        _title.value =
            page.getString(R.string.level) +
                    (difficulty * Config.LEVELS_PER_DIFFICULTIES + levelId + 1).toString() +
                    "\n" + page.getString(R.string.difficulty) + ": " +
    ...
    private fun wonLevelAgain() {
        _topText.value =
            page.getString(R.string.finished_level_again) + currentGame.getBestScoreString()
        _bottomText.value = ""
    }

    private fun wonFirstTimeLevel() {
        _topText.value =
            page.getString(R.string.finished_level) + currentGame.getBestScoreString()

        if ((passedLevels + 1) % Config.LEVELS_PER_DIFFICULTIES == 0) {
            if (passedLevels == finishedGameAt) {
                _bottomText.value = page.getString(R.string.finished_all_levels)
            } else {
                _bottomText.value =
                    page.getString(R.string.difficulty) +
                            Config.DIFFICULTIES_NAMES[(passedLevels / Config.LEVELS_PER_DIFFICULTIES) + 1] +
                            " " + page.getString(R.string.difficulty_unlocked)
                _nextLevelAvailable.value = true
            }
        } else {
            _bottomText.value =
                page.getString(R.string.level) + (passedLevels + 2).toString() +
                        " " + page.getString(R.string.level_unlocked)
            _nextLevelAvailable.value = true
        }
